I'll research GitHub's official documentation to reconstruct its logical interfaces and classify each by access channel. Let me work through the provider's own sources.REST and GraphQL confirmed as distinct Programmatic interfaces. Now webhooks, CLI, and the first-party client surfaces.Webhooks confirmed as Event. Now the CLI, first-party clients (mobile, desktop), and the AI surfaces.GitHub CLI confirmed as command-based (Programmatic); its interactive prompts are parameter collection, not a persistent TUI. Now the mobile and desktop clients, then the AI surfaces.GitHub Mobile confirmed as a first-party Interactive client. Now GitHub Desktop, then the Copilot Chat and MCP surfaces.GitHub Desktop confirmed as a first-party Interactive GUI client. Now the AI surfaces: Copilot Chat and the GitHub MCP server.Copilot Chat confirmed as a natural-language (Conversational) interface. Now the GitHub MCP server as a standalone interface, and Git remote operations.The GitHub MCP server appears in GitHub's official docs. Let me confirm its status as a first-party interface and its remote endpoint, plus check Git remote operations and the email/notifications inbound path.The MCP server is GitHub's official first-party server exposing discoverable toolsets (Agentic). Now Git remote operations and whether email reply to notifications is a supported inbound interface.Git remote operations confirmed as a Programmatic interface. Let me verify the current status of the reply-by-email inbound path before disposing of it in the ledger.# HARVEY4API: análisis de interfaces y canales de acceso de GitHub
